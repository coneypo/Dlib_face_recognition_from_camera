                ...
                if self.current_frame_face_cnt == self.last_frame_faces_cnt:
                    ...
                    if self.current_frame_face_cnt ==1:
                        if self.reclassify_interval_cnt==self.reclassify_interval:
                            print("   >>> scene 1.1 需要对于当前帧重新进行人脸识别 (Re-classification for current frame)")
                            ...
                            for k in range(len(faces)):
                                ...
                                # c. 对于某张人脸，遍历所有存储的人脸特征 (For every faces detected, compare the faces in the database)
                ...
                # 4.2 当前帧和上一帧相比发生人脸数变化 (If face cnt changes, 1->0 or 0->1)
                    ...
                    if self.current_frame_face_cnt == 1:
                        ...
                        for k in range(len(faces)):
                            ...
                            # c. 对于某张人脸，遍历所有存储的人脸特征 (For every face detected, compare with faces in the database)
                            for i in range(len(self.features_known_list)):
